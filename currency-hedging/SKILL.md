...
2. **脚本调用**：必须通过运行 `skills/currency-hedging/scripts/calculate_fx.py` 来获取精确计算结果。
    - 命令格式：`python3 skills/currency-hedging/scripts/calculate_fx.py [amount] [from_cur] [to_cur]`
3. **输出规范**：
    - 必须先列出当前的“模拟汇率”。
